...
{
//	WriteFile( GetStdHandle( STD_OUTPUT_HANDLE ), "uuuu\n", 5, NULL, NULL );
    free(p);
}
...
void start_logging(void)
{
  ...
  //  GetTempPath( sizeof(temp) - 8, (LPWSTR)temp );
...
int handle_host_request(char *service, transport_type ttype, char* serial, int reply_fd, asocket *s)
{
    ...
    if(!strcmp(service, "kill")) {
		D("adb server killed by remote request\n");
...
struct MyTrace {
	...
	MyTrace()
	{
		...
		//estrm = freopen("adb_log.txt", "a+", stderr);
		D("log begin(%s)...\n", name);
	}
